...
the automation closes any previous bot PRs, rebases the branch onto the latest
`main`, and attempts to merge the fresh update into the base branch. If GitHub
reports a conflict during the rebase or merge, the workflow leaves a comment
explaining what happened so you can resolve it manually before the next
scheduled run.
